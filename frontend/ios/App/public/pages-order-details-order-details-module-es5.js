...
var OrderDetailsPage = /** @class */ (function () {
    function OrderDetailsPage(formBuilder, progressIndicatorService, authService, route, router, navController, orderService, productService, paymentService, iab) {
    ...
    OrderDetailsPage.prototype.ngOnInit = function () {
        ...
        this.route.paramMap.subscribe(function (params) {
            if (params.get('orderId') === null) {
                ...
                _this.orderId = params.get('orderId');
                _this.displayOrderInformation();
            }
        });
    ...
    OrderDetailsPage.prototype.array = function (n) {
        var arr = Array(n);
        return Array.from(arr.keys()).map(function (ind) { return ind + 1; });
    };
    OrderDetailsPage.prototype.onRatingChanged = function (n) {
        this.rating = n;
    };
    OrderDetailsPage.prototype.fillTo = function (n) {
        this.filledStars = n;
    };
    OrderDetailsPage.prototype.acceptOrder = function () {
        var _this = this;
        this.orderService.accept(this.orderId).subscribe(function (data) {
            _this.displayOrderInformation();
        }, function (err) {
            console.log(err);
            _this.progressIndicatorService.presentToast('Order could not be rejected. Please try again.', 'danger');
        });
    };
    OrderDetailsPage.prototype.onSubmitReview = function () {
        var _this = this;
        ...
        this.productService.addReview(val).subscribe(function (data) {
            _this.reviewForm.reset();
            _this.progressIndicatorService.presentToast('Review successfully added');
            _this.displayOrderInformation();
        }, function (error) {
    ...
    OrderDetailsPage.prototype.rejectOrder = function () {
        var _this = this;
        this.orderService.reject(this.orderId).subscribe(function (data) {
            _this.displayOrderInformation();
        }, function (err) {
            console.log(err);
            _this.progressIndicatorService.presentToast('Order could not be rejected. Please try again.', 'danger');
    ...
    OrderDetailsPage.prototype.payOrder = function () {
        var _this = this;
        try {
            this.paymentService.createPayment(this.orderId).subscribe(function (data) {
                var link = data.payment.links[1].href;
                _this.paymentToken = data.token;
                localStorage.setItem('paymentToken', _this.paymentToken);
                if (!document.URL.startsWith('http')) {
                    var browser = _this.iab.create(link);
                    browser.show();
                }
                else {
    ...
    OrderDetailsPage.prototype.displayOrderInformation = function () {
        var _this = this;
        this.orderService.getOrderById(this.orderId)
            .subscribe(function (data) {
            _this.order = data;
            if (_this.order.seller._id != _this.authService.getId() && _this.order.buyer._id != _this.authService.getId() && !_this.authService.isAdmin())
                _this.navController.pop();
            var sellerId = _this.order.seller._id;
            var userId = _this.authService.getId();
            _this.isSeller = sellerId == userId ? true : false;
    ...
    OrderDetailsPage.prototype.onSubmitMessage = function () {
        var _this = this;
        ...
        this.orderService.sendMessage(body).subscribe(function (data) {
            _this.chatForm.reset();
            _this.displayOrderInformation();
        }, function (error) {
            console.log(error.error.error);
            _this.progressIndicatorService.presentToast(error.error.error, 'danger');
        });
    };
    OrderDetailsPage.prototype.returnStatusMessage = function (message, order) {
        if (!message.message)
            return "Error";
        var text = message.message;
        var args = message.args;
        var sellerArticle = this.isSeller ? "your" : "the";
        var buyerArticle = !this.isSeller ? "your" : "the";
