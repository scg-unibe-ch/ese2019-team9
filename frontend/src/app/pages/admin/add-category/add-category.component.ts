...
export class AddCategoryComponent implements OnInit {
	...
	onSubmitForm() {
		...
		promise.then(
			(data) => {
				...
				this.progressIndicatorService.dismissLoadingIndicator();
				this.progressIndicatorService.presentToast('Category not updated', 2000, 'danger');
				console.log(reason);
			}).then(() => {
				this.categoryService.getCategories().subscribe(
					data => {
	...
	updateCategory(form: FormGroup): Promise<any> {
		return new Promise((resolve, reject) => {
			...
			body += `}`;
			if (firstLine) {
				this.progressIndicatorService.presentToast('Nothing has been changed', 2000, "warning");
				return;
			};
	...
	onDeleteCategory(category: Category) {
		...
		promise.then((shouldDelete)=> {
			...
			this.categoryService.deleteCategory((category as any)._id).subscribe(
				(data) => {
					this.progressIndicatorService.dismissLoadingIndicator();
					this.progressIndicatorService.presentToast('Sucessfully deleted Category', 2000);
					this.categoryService.getCategories().subscribe(
						data => {
							this.currentCategories = data;
							if (!this.updateNotCreate) this.setUniqueValidator();
					...
					this.progressIndicatorService.dismissLoadingIndicator();
					this.progressIndicatorService.presentToast('Could not delete Category', 2000, 'danger');
					console.log(error);
				}
			);
		});
