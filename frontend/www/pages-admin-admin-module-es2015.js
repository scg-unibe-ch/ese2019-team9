...
class Category {
    constructor(name, slug, image, id, parentId, parentSlug) {
        this.name = name;
        this.slug = slug;
        this.image = image;
        if (id)
...
let AddCategoryComponent = class AddCategoryComponent {
    constructor(categoryService, formBuilder, progressIndicatorService, alertController) {
    ...
    updateCategory(form) {
        return new Promise((resolve, reject) => {
            ...
            ;
            this.categoryService.updateCategory(this.selectedCategory._id, body, this.imageFile).subscribe((data) => {
                resolve(data);
            }, (err) => {
                reject(err);
            });
    ...
    createCategory(values) {
        return new Promise((resolve, reject) => {
            ...
            const category = new src_app_models_category__WEBPACK_IMPORTED_MODULE_3__["Category"](values.name, values.slug, '');
...
const UniqueValidator = (objArr, fieldname) => {
    return (control) => {
        return new Promise((resolve) => {
            ...
            while (workingArray.length > 0) {
                ...
                if (workingArray[0].subcategories) {
                    for (let i = 0; i < workingArray[0].subcategories.length; i++) {
...
let ManageOffersComponent = class ManageOffersComponent {
    constructor(productService, progressIndicatorService, notificationService, filterService) {
    ...
    deleteOffer(productId, productName, sellerId) {
    ...
    verifyOffer(productId, productName, sellerId) {
        this.progressIndicatorService.presentLoading('Loading...');
        this.productService.verifyProduct(productId).subscribe(data => {
            this.progressIndicatorService.dismissLoadingIndicator();
    ...
    reviseOffer(productId, productName, sellerId) {
        this.progressIndicatorService.presentLoading('Loading...');
        this.productService.reviseProduct(productId).subscribe(data => {
            this.progressIndicatorService.dismissLoadingIndicator();
    ...
    notifySeller(productId, productName, sellerId, notificationType) {
        let link = '/home';
        let message = '';
        ...
        setTimeout(() => {
            this.notificationService.notifySingleUser(sellerId, body).subscribe(data => {
            }, err => {
    ...
    getDateString(stringRepresentation) {
        return new Date(stringRepresentation).toUTCString();
    }
};
ManageOffersComponent.ctorParameters = () => [
