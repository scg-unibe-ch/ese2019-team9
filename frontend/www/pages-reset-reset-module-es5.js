...
var ResetPage = /** @class */ (function () {
    function ResetPage(formBuilder, authService, activatedRoute, progressIndicatorService) {
    ...
    ResetPage.prototype.onSubmitReset = function () {
        ...
        this.activatedRoute.queryParamMap.subscribe(function (queryParams) {
            _this.authService.resetPassword(queryParams.get('token'), val.password)
                ...
                if (data.status === 200) {
                    _this.progressIndicatorService.presentToast('Your password has been reset', 'success', 4000);
                    _this.resetForm.reset();
                }
            }, function (error) {
                console.log(error);
                _this.progressIndicatorService.presentToast('Invalid request', 'danger', 4000);
            });
        });
    };
