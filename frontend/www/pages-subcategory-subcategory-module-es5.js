...
var SubcategoryPage = /** @class */ (function () {
    function SubcategoryPage(route, productService, categoryService, filterAndSearchService, progressIndicatorService) {
        this.route = route;
    ...
    SubcategoryPage.prototype.updateProducts = function () {
        var _this = this;
        return this.productService.getProductsById(this.subcategory._id).then(function (products) {
            // @ts-ignore
            _this.allProducts = products;
            _this.products = _this.applyFilters(_this.allProducts);
            _this.getPriceSpan();
            _this.selectFeaturedProducts();
        });
    };
    SubcategoryPage.prototype.applyFilters = function (products) {
        var _this = this;
        this.filterargs = [];
        Object.keys(this.filterSettings).forEach(function (key) {
            if (_this.filterSettings[key] !== '') {
                _this.filterargs.push(_this.filterSettings[key]);
            }
        });
    ...
    SubcategoryPage.prototype.filterDidNotChange = function (arr1, arr2) {
        if (arr1 === arr2) {
            return true;
        }
    ...
    Object.defineProperty(SubcategoryPage.prototype, "filterargsToDisplay", {
        get: function () {
            var _this = this;
            var detailNames = new Map([
                ['price', new Map([
                        ['>=', 'minPrice'],
                        ['<=', 'maxPrice']
                    ])]
            ...
            var filterObjArray = [];
            this.filterargs.forEach(function (el) {
                var filterObj = _this.filterAndSearchService.filterToObject(el);
                var name = filterObj.name;
                var operator = filterObj.operator;
                var map = (detailNames.has(name)) ? detailNames.get(name) : undefined;
                var detailName = (map && map.get(operator)) ? map.get(operator) : filterObj.name;
                if (name == 'price') {
                ...
                filterObjArray.push({ name: filterObj.name, operator: filterObj.operator, value: filterObj.value, detailName: detailName });
            });
    ...
    Object.defineProperty(SubcategoryPage.prototype, "selectedPrice", {
        get: function () {
    ...
    SubcategoryPage.prototype.removeFilter = function (filter) {
        var _this = this;
        var resetFilterInput = new Map([
            ['rating', function () { _this.filledStars = 0; }],
            ['maxPrice', function () {
                    document.querySelector('#maxPriceInput').value = _this.priceSpan.upper.toString();
                    var element = document.querySelector('#priceRangeInput');
                    _this.setRangeValueTo(element.value.lower, _this.priceSpan.upper);
                }],
            ['minPrice', function () {
                    document.querySelector('#minPriceInput').value = _this.priceSpan.lower.toString();
                    var element = document.querySelector('#priceRangeInput');
                    _this.setRangeValueTo(_this.priceSpan.lower, element.value.upper);
                }]
        ]);
    ...
    SubcategoryPage.prototype.setRangeValueTo = function (lower, upper) {
        var element = document.querySelector('#priceRangeInput');
        element.value = { lower: lower, upper: upper };
    };
    SubcategoryPage.prototype.onRatingFilterChanged = function (n) {
        this.filterSettings.rating = ">=;rating;" + n;
        this.products = this.applyFilters(this.allProducts);
    };
    SubcategoryPage.prototype.onPriceSliderChange = function (evt) {
        var newPrice = evt.target.value;
        this.onMinPriceChange(newPrice.lower);
        this.onMaxPriceChange(newPrice.upper);
    };
    SubcategoryPage.prototype.onMinPriceChange = function (n) {
        var input = document.querySelector('#minPriceInput');
    ...
    SubcategoryPage.prototype.onMaxPriceChange = function (n) {
        var input = document.querySelector('#maxPriceInput');
        if (!Object(util__WEBPACK_IMPORTED_MODULE_7__["isUndefined"])(n)) {
            if (input.value !== n.toString()) {
                input.value = n.toString();
            }
    ...
    SubcategoryPage.prototype.array = function (n) {
        var arr = Array(n);
        return Array.from(arr.keys()).map(function (ind) { return ind + 1; });
    };
    SubcategoryPage.prototype.fillTo = function (n) {
        this.filledStars = n;
    };
    SubcategoryPage.prototype.getPriceSpan = function () {
        var sorted = this.filterAndSearchService.sort(this.products.slice(), '+price');
        if (sorted.length === 0) {
            return { lower: 0, upper: 1000 };
        }
        this.priceSpan = { lower: sorted[0]['price'], upper: sorted[sorted.length - 1]['price'] };
    };
    SubcategoryPage.prototype.onSortChange = function (evt) {
