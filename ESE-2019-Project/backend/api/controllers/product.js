...
exports.getProducts = (req, res, next) => {
    const search = req.userData.admin ? {} : {
        verified: true
    };
    Product.find(search)
...
/**©
 * Check whether user has bought certain product or not
 */
exports.hasUserBought = (req, res, next) => {
    Order.find({
            $and: [{
        ...
        .then(docs => {
            res.status(200).json({
                hasBought: docs.length > 0 ? true : false
            });
        })
        .catch(err => {
            res.status(500).json({
